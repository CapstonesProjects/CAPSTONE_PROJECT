...
// Ensure the FullNameSender is set in the session
if (!isset($_SESSION['FullNameSender'])) {
    // Retrieve the full name of the sender (admin) from the database
    $adminNumber = $_SESSION['AdminNumber'];
    $query = "SELECT CONCAT(FirstName, ' ', COALESCE(MiddleName, ''), ' ', LastName) AS FullNameSender FROM admin WHERE AdminNumber = ?";
    ...
    if ($admin) {
        $_SESSION['FullNameSender'] = $admin['FullNameSender'];
    } else {
        $_SESSION['FullNameSender'] = 'Unknown Sender';
    }
}
?>
...
<div id="composeWindow" class="fixed bottom-0 right-0 bg-white shadow-lg rounded-lg p-4 w-1/4 h-auto hidden">
    ...
    <form method="POST" action="../phpfiles/sendmessage_admin.php" enctype="multipart/form-data">
        <div class="mb-2">
        ...
        <input type="hidden" name="fullNameSender" id="fullNameSender" value="<?php echo htmlspecialchars($_SESSION['FullNameSender']); ?>">
