...
<div class="overflow-y-auto" style="max-height: 840px;">
    <ul>
        <?php while ($message = $result->fetch_assoc()): ?>
            <li class="flex items-center border-y hover:bg-gray-200 px-2 sent-message-container" data-id="<?php echo htmlspecialchars($message['MessageID'] ?? ''); ?>" data-date="<?php echo htmlspecialchars($message['created_at']); ?>" data-receiver="<?php echo htmlspecialchars($message['FullNameReceiver'] ?? 'Unknown Receiver'); ?>" data-subject="<?php echo htmlspecialchars($message['subject'] ?? 'No Subject'); ?>" data-body="<?php echo htmlspecialchars($message['body'] ?? 'No Body'); ?>">
...
<script>
    ...
    const detailsAttachments = document.getElementById('sent-details-attachments');
